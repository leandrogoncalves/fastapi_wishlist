...
class ProductService:
    ...
    def __init__(self):
        self.repository = self.__get_product_repository()

    @inject()
    def __get_product_repository(self, repository: ProductRepositoryAbstract):
        return repository

    def __get_product(self, product_model: ProductModel) -> Product:
        return Product(
            id=product_model.id,
    ...
    def __validate_product(self, product_model: ProductModel) -> None:
    ...
    async def get_all_paginated(self, page: int) -> ProductPaginated:
        ...
        product_data = [self.__get_product(product) for product in data]
    ...
    async def get_by_id(self, product_id: str) -> Optional[Product]:
        ...
        self.__validate_product(product_model)
        return self.__get_product(product_model)
    ...
    async def store(self, product: Product) -> ProductModel:
        ...
        return self.__get_product(await self.repository.create(new_product))
    ...
    async def update(self, product_id: str, product: Product) -> Product:
        ...
        self.__validate_product(product_founded)
        if product.title:
        ...
        return self.__get_product(await self.repository.update(product_founded))
